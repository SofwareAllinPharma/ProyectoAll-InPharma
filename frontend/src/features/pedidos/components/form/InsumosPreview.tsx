    ...
    <div>
      ...
      {!selected || !convGramos ? (
        <div className="p-3 border border-gray-200 rounded-md bg-gray-50 min-h-[80px] flex items-center justify-center">
          <div className="text-sm text-gray-500 text-center">Seleccione un producto y cantidad para ver los insumos requeridos.</div>
        </div>
      ) : insumos.length === 0 ? (
        <div className="p-3 border border-gray-200 rounded-md bg-gray-50 min-h-[80px] flex items-center justify-center">
          <div className="text-sm text-gray-500 text-center">No hay insumos en la fórmula del producto.</div>
        </div>
      ) : (
        <div className="border border-gray-200 rounded-md bg-white overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {insumos.map((i, idx) => (
                <tr key={i.nombre} className={`hover:bg-gray-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                  <td className="px-3 py-2 text-gray-900">{i.nombre}</td>
                  <td className="px-3 py-2 text-right font-medium text-[#5d5448]">{formatQty(i.cantidad)} g</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {error && <p className="mt-2 text-xs text-red-600 bg-red-50 p-2 rounded-md border border-red-200">{error}</p>}
    </div>
  );
};
