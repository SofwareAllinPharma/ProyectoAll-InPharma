...
export default function ProductInfoCollapse({
  formula,
  pesoNeto,
  cantPorciones,
}: Props) {
  const distribution = useMemo(() => {
    ...
    return `${porcionesCompletas} porciones completas + ${pesoParcial.toFixed(
      4
    )}g`;
  ...
  return (
    ...
    >
      <div className="text-sm text-gray-700 space-y-1">
        <p>
          <strong>Fórmula:</strong> {formula.nombre}
        </p>
        <p>
          <strong>Peso por porción:</strong> {formula.porcion || 0}g
        </p>
        <p>
          <strong>Peso neto total:</strong>{" "}
          {Number.isFinite(pesoNeto) ? pesoNeto.toFixed(4) + "g" : "N/A"}
        </p>
        <p>
          <strong>Distribución:</strong> {distribution}
        </p>
      </div>
    </Collapsible>
  );
