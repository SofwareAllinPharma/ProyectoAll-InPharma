...
export default function ConfirmarCambioEstadoModal({ open, onClose, from, to, onConfirm }: { open: boolean; onClose: () => void; from: string; to: string; onConfirm: (args: { responsable: string; observaciones?: string }) => Promise<void> | void }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      setLoading(true);
      await onConfirm({ responsable: responsable.trim(), observaciones: obs.trim() || 'No Aplica' });
      onClose();
    } finally {
      setLoading(false);
    }
  ...
  <Modal open={open} onClose={onClose} className="z-[11000]" containerClass="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4">
      <div className="flex flex-col h-full">
        ...
        <div className="px-5 pb-5 flex gap-2 justify-end">
          ...
          <Button onClick={handleConfirm} disabled={loading || !responsable.trim()}>{loading ? 'Guardando…' : 'Confirmar'}</Button>
        </div>
      </div>
    </Modal>
  );
