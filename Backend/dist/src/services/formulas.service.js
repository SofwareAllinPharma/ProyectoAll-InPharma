...
const formulas_repository_1 = require("../repositories/formulas.repository");
...
class FormulasService {
    constructor() {
        this.repo = new formulas_repository_1.FormulasRepository();
    }
    async list(params) {
    ...
    async createFormula(dto) {
        // Validación: no permitir insumos repetidos
        const insumoIds = dto.insumos.map((i) => i.idInsumo);
        const insumoSet = new Set(insumoIds);
        if (insumoIds.length !== insumoSet.size) {
            throw new Error("No se puede agregar el mismo insumo más de una vez en la fórmula.");
        }
        // Validación de nombre único solo entre fórmulas activas
        const nombreExistente = await prisma_1.prisma.formula.findFirst({
            where: {
        ...
        if (nombreExistente)
            throw new Error("El nombre de la fórmula ya existe");
        // Validaciones
        ...
        const insumosIds = dto.insumos.map((i) => i.idInsumo);
        ...
        const porcion = dto.insumos.reduce((acc, curr) => acc + Number(curr.cantidadInsumo), 0);
        if (porcion <= 0)
        ...
        for (const fi of dto.insumos) {
            ...
            if (!insumo)
                throw new Error(`Insumo ${fi.idInsumo} no encontrado`);
        ...
        // Redondear
        Object.keys(nutrientes).forEach((k) => (nutrientes[k] = round(nutrientes[k])));
        // kcalorias
        let kcalorias = 0;
        for (const fi of dto.insumos) {
            const insumo = insumos.find((i) => i.id === fi.idInsumo);
    ...
    async updateFormula(id, dto) {
        ...
        if (nombreExistente)
            throw new Error("El nombre de la fórmula ya existe");
        const formula = await prisma_1.prisma.formula.findUnique({ where: { id } });
        if (!formula)
    ...
    async cloneFormula(id, dto) {
        // Obtener la fórmula original
        const original = await prisma_1.prisma.formula.findUnique({ where: { id } });
        if (!original)
            throw new Error("Fórmula original no encontrada");
