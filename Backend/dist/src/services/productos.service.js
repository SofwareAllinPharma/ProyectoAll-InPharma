...
class ProductosService {
    constructor() {
        this.repo = new productos_repository_1.ProductosRepository();
    }
    async list(params) {
        return this.repo.findAll(params);
    }
    async detail(idProducto) {
    ...
    async createProducto(dto) {
        ...
        const existingProduct = await prisma_1.prisma.producto.findFirst({
            where: {
        ...
        if (!dto.idFormula)
            throw new Error("Debe indicar una fórmula");
        ...
        if (pesoNeto <= 0)
            throw new Error("El peso neto debe ser mayor a 0");
    ...
    async updateProducto(idProducto, dto) {
    ...
    async softDeleteProducto(idProducto) {
        ...
        const existe = await prisma_1.prisma.pedido.findFirst({
            where: {
                idProducto,
        ...
        if (existe) {
            throw new Error("No se puede eliminar el producto: tiene pedidos no cancelados");
        }
